...
def preprocess_embedding(  # adds placeholder tokens for the sequence of tokens from the Point Encoder: <point_patch><point_patch>...
    ...
    for source in sources:
        for sentence in source:
            replace_token = default_point_patch_token * point_token_len     # point_token_len=1 in LLaNA, point_token_len=513 in PointLLM, default_point_patch_token='<point_patch>'
...
@dataclass
class DataCollatorForSpatialDataset(object):
...
@dataclass
class DataCollatorForSpatialDataset_Eval(object):
    """Collate examples for mixed dataset with text and nf2vec data."""
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        # Preserve conversations for ground truth extraction during evaluation
