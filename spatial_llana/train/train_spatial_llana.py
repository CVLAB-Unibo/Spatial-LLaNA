...
import transformers
from spatial_llana.train.spatial_llana_trainer import Spatial_LLaNATrainer

from spatial_llana import conversation as conversation_lib
from spatial_llana.model import *
from spatial_llana.data import make_spatial_data_module

# * logger
from spatial_llana.utils import build_logger
...
def train():
    ...
    if training_args.model_debug:
        ...
        model = Spatial_LLaNA._from_config(config)
    else:
        model = Spatial_LLaNA.from_pretrained(
    ...
    if not training_args.stage_2:
        ...
        model.initialize_tokenizer_weights2space_config_config(tokenizer=tokenizer, device=training_args.device, fix_llm=training_args.fix_llm)
    else:
        # * stage2
        model.initialize_tokenizer_weights2space_config_wo_embedding(tokenizer=tokenizer)
    
    weights2space_config_config = model.get_mweightsweights2space_config_config

    data_args.point_token_len = weights2space_config_config['point_token_len']
    ...
    data_module = make_spatial_data_module(tokenizer=tokenizer,
                                                    data_args=data_args)

    trainer = Spatial_LLaNATrainer(model=model,
                    tokenizer=tokenizer,
                    args=training_args,
                    **data_module)
