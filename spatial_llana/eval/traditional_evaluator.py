...
class TraditionalMetricEvaluator():
    def __init__(self, inputs, output_dir, output_file, device):
        self.results = inputs
        ...
        self.simcse_model = AutoModel.from_pretrained(f"{HF_HOME}/hub/models--princeton-nlp--sup-simcse-roberta-large/snapshots/96d164d9950b72f4ce179cb1eb3414de0910953f").to(self.device)
    ...
    def evaluate_result(self, result):
        ...
        model_output = result['model_output'][:200]
        ...
        simcse_similarity = 1 - cosine(embeddings[0].cpu(), embeddings[1].cpu()) # * consine actually calculates consine distance, which is 1 - consine similarity
...
def start_evaluation(results, output_dir, output_file,
                        parallel=True, num_workers=20, device=None):
    """
    Args:
    ...
    evaluator = TraditionalMetricEvaluator(results, output_dir, output_file, device) 
    evaluator.evaluate()
...
if __name__ == "__main__":
    ...
    start_evaluation(results=args.results_path, output_dir=args.output_dir, output_file=output_file, device=args.device)
