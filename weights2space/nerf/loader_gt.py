...
### from nerfacc_nerf2vec in LEONARDO ###
def _load_renderings_shapenet_objaverse(root_fp: str, split: str):
    """Load images from disk."""
    
    if 'objaverse' in root_fp:
        h, w = (512, 512)
    else:
        h, w = (224, 224)
        
    data_dir = os.path.join(root_fp, split)
    with open(
...
class ShapeNeRFObjaNeRFLoaderGT(torch.utils.data.Dataset):
    OPENGL_CAMERA = True
    NEAR, FAR = 2.0, 6.0
    ...
    def __init__(
        ...
        far: float = None,
        device: str = "cuda:0",
        weights_file_name: str = "mlp_128/nerf_weights.pth",
        training: bool = True
    ):
        ...
        self.camtoworlds, self.focal = _load_renderings_shapenet_objaverse(data_dir, split)
...
def _load_renderings_objaverse(root_fp: str, split: str):
    """Load images from disk."""
    ...
    for i in range(len(meta["frames"])):
        ...
        #images.append(rgba)
    end_load = time.time()
    #print('time to load images:', end_load - start_load)
...
class ObjaNeRFLoaderGT(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.weights_file_path = os.path.join(data_dir, weights_file_name)
        start_load = time.time()
        _, self.camtoworlds, self.focal = _load_renderings_objaverse(data_dir, split)
        end_load = time.time()
        #print('Time to load renderings: ', end_load - start_load)
        ...
        end_init = time.time()
        #print(f"Time to initialize ObjaNeRFLoaderGT: {end_init - start_init}")
