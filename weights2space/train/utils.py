...
def progress_bar(
    ...
    Args:
        ...
        disable: whether to turn off the progress bar.
            ...
            if radiance_field.training or batch_size > 1:
                ...
                for batch_idx in range(batch_size):
                    ...
                    if background_indices is not None:
                        grid_coords = occupancy_grid.grid_coords[background_indices[batch_idx]] ### WORK HERE
                        bg_positions = (
...
def render_image_shapenerf_objanerf(
    # scene
    radiance_field: torch.nn.Module,             
    embeddings: torch.Tensor,
    ...
    for i in range(0, num_rays, chunk):
        ...
        with torch.no_grad():
            for batch_idx in range(batch_size):
                data_dir = data_dirs[batch_idx]
                if 'objaverse' in data_dir:
                    occupancy_grid = occupancy_grid_objanerf
                else:
                    occupancy_grid = occupancy_grid_shapenerf
                ...
                n_total_cells = cfg.GRID_NUMBER_OF_CELLS_OBJANERF if 'objaverse' in data_dir else cfg.GRID_NUMBER_OF_CELLS_SHAPENERF
                grid_weights['occs'] = torch.empty([n_total_cells])

                if grid_weights != None and occupancy_grid != None:
                    dict = {
                        '_roi_aabb': grid_weights['_roi_aabb'],
                        '_binary': grid_weights['_binary'],
                        'resolution': grid_weights['resolution'],
                        'occs': grid_weights['occs'],
                    }
                    # occupancy_grid = None
                    occupancy_grid.load_state_dict(dict)
...
def render_image_shapenerf_objanerf_triplane(
    # scene
    radiance_field: torch.nn.Module,             
    ...
    rays_shape = rays.origins.shape # (1, 512, 3)
    if len(rays_shape) == 4:
        batch_size, height, width, _ = rays_shape
        num_rays = height * width
        rays = namedtuple_map(
        ...
        with torch.no_grad():
            ...
            if radiance_field.training or batch_size > 1:
                for batch_idx in range(batch_size):
                    ...
                    # PADDING: do nothing, the padding will be added later
                    if b_positions[batch_idx].size(0) < MAX_SIZE:   # b_positions (33620)
                        pass
            ...
            b_t_ends = torch.stack(b_t_ends, dim=0)
            b_ray_indices = torch.stack(b_ray_indices, dim=0)
            b_positions = torch.stack(b_positions, dim=0)   # (B, 35000, 3)
            
        # ####################
        # VOLUME RENDERING
        # ####################        
        
        curr_rgb, curr_sigmas = radiance_field(triplane, b_positions)   # curr_rgb (B, 35000, 3)    curr_sigmas (B, 35000, 1)
...
def render_image_GT(
    ...
    for batch_idx in range(batch_size):
        ...
        curr_ngp_mlp_weights = {
            ...
            #'mlp_base.params': ngp_mlp_weights['mlp_base.params']              # uncomment when running distributed training
        }
        ...
        for i in range(0, num_rays, chunk):
            ...
            try:
                ...
                print('ERROR IN RAY MARCHING')
                print('Error: ', e)
                print('broken batch_idx: ', batch_idx)
                broken_batch_idxs.append(batch_idx)
                continue
            rgb, opacity, depth = rendering(
                t_starts,
                t_ends,
        ...
        if not is_rendering_full_image:
            try:
                ...
                print('ERROR IN SAMPLING PIXELS')
                print('Error: ', e)
                print('broken batch_idx: ', batch_idx)
                broken_batch_idxs.append(batch_idx)
                continue
    ...
    return pixels, b_opacities, filtered_rays, broken_batch_idxs

@torch.no_grad()
def render_image_GT_shapenerf_objanerf(
    # scene
    radiance_field: torch.nn.Module,
    occupancy_grid_shapenerf: OccupancyGrid,
    occupancy_grid_objanerf: OccupancyGrid,
    ...
    for batch_idx in range(batch_size):
        ...
        curr_ngp_mlp_weights = {
            'mlp_base.params': ngp_mlp_weights['mlp_base.params'][batch_idx]
            #'mlp_base.params': ngp_mlp_weights['mlp_base.params']
        }
        radiance_field.load_state_dict(curr_ngp_mlp_weights)
        occupancy_grid.load_state_dict(curr_grid_weights)

        curr_rays = Rays(origins=rays.origins[batch_idx], viewdirs=rays.viewdirs[batch_idx])    # origins (55000, 3)    viewdirs (55000, 3)
        ...
        for i in range(0, num_rays, chunk):
            ...
            chunk_results = [rgb, opacity, depth, len(t_starts)]    #rgb (55000, 3) opacity (55000, 1)  depth (55000, 1)    t_start (248957, 1)
            results.append(chunk_results)
        colors, opacities, _, _ = [
            torch.cat(r, dim=0) if isinstance(r[0], torch.Tensor) else r
            for r in zip(*results)
        ...
        if not is_rendering_full_image:
            try:
                colors, opacities, indices = sample_pixels_uniformly(opacities, colors, max_elements)   # INPUT: opacities (55000, 1)  colors (55000, 3)   max_elements=512
                filtered_rays.origins[batch_idx] = rays.origins[batch_idx][indices]
            except Exception as e:
                print('ERROR IN SAMPLING PIXELS')
                print('Error: ', e)
        ...
        pixels[batch_idx] = colors.view((*rays_shape[1:-1], -1))            # pixels[batch_idx] (512, 3) 
        b_opacities[batch_idx] = opacities.view((*rays_shape[1:-1], -1))    # b_opacities[batch_idx] (512, 3)
